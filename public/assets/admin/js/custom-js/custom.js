...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- Your image preview code start ---
    // Global array to track selected files
    window.allSelectedFiles = []; // use window to make it globally accessible
    ...
    window.previewImages = function () {
        ...
        for (let file of files) {
            if (!allowedTypes.includes(file.type)) {
                alert('Only JPEG and PNG images are allowed (jpeg, jpg, png)');
                fileInput.value = ''; // Clear input on invalid type
                return;
            }
        }

        // Append new files to allSelectedFiles
        window.allSelectedFiles = window.allSelectedFiles.concat(Array.from(files));

        // Clear current preview container
        imagePreview.innerHTML = ''; 

        // Loop through all selected files and preview them
        window.allSelectedFiles.forEach((file, index) => {
            const reader = new FileReader();

            reader.onload = function(e) {
                const imgContainer = document.createElement('div');
                imgContainer.classList.add('foto-grid-image');
                ...
                //     imagePreviewGrid.style.background = '#fff';
                //     imagePreviewGrid.style.padding = '15px';
                //     imagePreviewGrid.style.border = '1px solid #008955';
                //     imagePreviewGrid.style.borderRadius = '5px';
                //     imagePreviewGrid.style.marginBottom = '20px';
                ...
                closeIcon.onclick = function() {
                    ...
                    // Remove file from allSelectedFiles
                    window.allSelectedFiles.splice(index, 1);
        ...
        // Update the file input element to reflect the currently selected files
        updateFileInput();
    ...
    function updateFileInput() {
        ...
        window.allSelectedFiles.forEach(file => {
    ...
    $('#aiDataForm').on('submit', function (e) {
        e.preventDefault();
    ...
    window.previewImagesEdit = function () {
        const fileInput = document.getElementById('fileInput1');
        const imagePreview = document.getElementById('imagePreview1');
    ...
    $('#editAiData').on('submit', function (e) {
        e.preventDefault();
    ...
    //  *************************store neutrition data start*********************************************************************** 


    $('#aiNeutritionForm').on('submit', function (e) {
        e.preventDefault();
        $('.error-message').remove();
        ...
        $.ajax({
            url: aiNeutritionStoreData,
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                $('#aiNeutritionForm')[0].reset();
                window.allSelectedFiles = [];
                document.getElementById('imagePreview1').innerHTML = '';
                //   $('#successMessage').removeClass('d-none').text('AI data succesvol opgeslagen.').fadeIn().delay(3000).fadeOut();
                //    window.location.href = aiRedirectUrl + '?success=1';
